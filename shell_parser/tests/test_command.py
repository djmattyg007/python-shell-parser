...
def test_missing_compulsory_data():
    with pytest.raises(TypeError, match=make_match("__init__() missing 2 required positional arguments: 'command' and 'descriptors'")):
        Command()

    with pytest.raises(TypeError, match=make_match("__init__() missing 1 required positional argument: 'descriptors'")):
        Command(command=Word("cmd"))


def test_invalid_next_command_data():
    with pytest.raises(InvalidCommandDataException, match=make_match("Next command operator set, but no next command set.")):
