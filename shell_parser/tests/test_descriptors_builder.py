...
def test_default_init():
    ...
    assert stdin.for_reading is True
    assert stdin.for_writing is False
    assert stdin.descriptor.is_default_file is True
    assert stdin.descriptor.target.is_stdin is True
    assert stdin.descriptor.target.is_stdout is False
    assert stdin.descriptor.target.is_stderr is False

    stdout = builder.descriptors[DESCRIPTOR_DEFAULT_INDEX_STDOUT]
    assert stdout.for_reading is False
    assert stdout.for_writing is True
    assert stdout.descriptor.is_default_file is True
    assert stdout.descriptor.target.is_stdin is False
    assert stdout.descriptor.target.is_stdout is True
    assert stdout.descriptor.target.is_stderr is False

    stderr = builder.descriptors[DESCRIPTOR_DEFAULT_INDEX_STDERR]
    assert stderr.for_reading is False
    assert stderr.for_writing is True
    assert stderr.descriptor.is_default_file is True
    assert stderr.descriptor.target.is_stdin is False
    assert stderr.descriptor.target.is_stdout is False
    assert stderr.descriptor.target.is_stderr is True
...
def test_custom_init(fds):
    ...
    for fd in fds:
        assert builder.descriptors[fd].descriptor.is_default_file is False
    ...
    for fd in default_fds:
        assert builder.descriptors[fd].descriptor.is_default_file is True
